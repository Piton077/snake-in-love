...
window.onload = function () {
    function replaceGiftToActualOne() {
        const giftBox = document.getElementById("gift_img")
        giftBox.style.opacity = '0'
        setTimeout(() => {
            giftBox.style.opacity = '100'
            giftBox.src = "./assets/photo.jpeg"
        }, 1000);
        gameover = true
    }
    ...
    function updateScore() {
        ...
        if (score < 5) {
            node.textContent = `${score} meses`
        } else {
            node.textContent = `Felices ${score} meses bb \uD83D\uDC95`
            replaceGiftToActualOne()
        }
    }
    ...
    function render() {
        ...
        if (gameover) {
            ...
            drawCenterText("Press space to start!", 0, canvas.height / 2, canvas.width);
        }
    }
